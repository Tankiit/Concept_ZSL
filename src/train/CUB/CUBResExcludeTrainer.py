...
def train_one_epoch(labels_start=0, scheduler=None):
    running_loss = 0.
    ...
    for i, data in enumerate(training_loader):
        ...
        labels = labels.to(device) - labels_start
...
model.classes = 5
model.predicate_matrix = torch.nn.Parameter(torch.randn(NUM_EXCLUDE, NUM_FEATURES, device="cuda"))

accuracy = Accuracy(task="multiclass", num_classes=NUM_EXCLUDE, top_k=1).to(device)

model_parameters = filter(lambda p: p.requires_grad, model.parameters())

optimizer = torch.optim.Adam(model_parameters, lr=3e-4, weight_decay=1e-5)
