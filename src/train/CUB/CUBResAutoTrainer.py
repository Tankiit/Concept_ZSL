...
model.fc = torch.nn.Linear(2048, NUM_FEATURES).to(device)
...
optimizer = SAM(list(model.parameters()) + list(loss_fn.parameters()), base_optimizer, lr=3e-4, weight_decay=1e-5)
scheduler = torch.optim.lr_scheduler.OneCycleLR(optimizer.base_optimizer, 2e-3, epochs=EPOCHS, steps_per_epoch=len(training_loader))
#scheduler = None
...
for epoch in tqdm(range(EPOCHS)):
    ...
    if best_stats["val_acc"] < avg_acc:
        ...
        torch.save(model.state_dict(), "CUBResAuto.pt")
        torch.save(loss_fn.state_dict(), "CUBResAutoLossFN.pt")
