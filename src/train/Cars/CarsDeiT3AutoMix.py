...
eps=1e-10
def loss_fn(out, mixed_labels, labels, predicate_matrix):
    ...
    loss_cl = entr_loss(diff.sum(dim=2), mixed_labels) # Is "out" a subset of its class' predicates?
...
def train_one_epoch(scheduler):
    ...
    for i, data in enumerate(training_loader):
        ...
        inputs, mixed_labels = cutmix_or_mixup(inputs, labels)
        inputs = inputs.to(device)
        mixed_labels = mixed_labels.to(device)
        labels = labels.to(device)
        
        outputs, commit_loss, predicate_matrix = model(inputs)
        loss = loss_fn(outputs, mixed_labels, labels, predicate_matrix) + commit_loss
...
for epoch in tqdm(range(EPOCHS)):
    ...
    with torch.no_grad():
        for i, vdata in enumerate(validation_loader):
            ...
            vloss = loss_fn(voutputs, vlabels, vlabels, predicate_matrix) + vcommit_loss
    ...
    if best_stats["val_acc"] < avg_acc:
        ...
        torch.save(model.state_dict(), "CarsDeiT3CutMixUp.pt")
