...
AwA2_dataset = AwA2Dataset(root='/storage/Animals_with_Attributes2/JPEGImages', transform=val_transform)

AwA2_loader = torch.utils.data.DataLoader(
        AwA2_dataset, batch_size=32, shuffle=True, num_workers=4)
...
def train_one_epoch(scheduler):
    ...
    for j, data in enumerate(AwA2_loader):
        if j == 10:
            break
        # Every data instance is an input + label pair
        inputs, labels = data["images"], data["labels"]
        ...
        loss = mse_loss(outputs, torch.zeros_like(outputs))
...
scheduler = torch.optim.lr_scheduler.OneCycleLR(optimizer.base_optimizer, 1e-4, epochs=EPOCHS, steps_per_epoch=len(training_loader)+10)
#scheduler = None
...
for epoch in tqdm(range(EPOCHS)):
    ...
    if best_stats["val_acc"] < avg_acc:
        ...
        torch.save(model.state_dict(), "CarsDeiT3Null.pt")
